...
<<fit-3>>=
aout3 <- aster(resp ~ varb + nsloc + ewloc + site * hdct,
    pred, fam, varb, id, root, data = redata)
summary(aout3)
@
...
fred <- summary(aout4)
dimnames(fred$coef)
fred$coef["siteEriley:hdct", "Estimate"] +
c(-1, 1) * crit * fred$coef["siteEriley:hdct", "Std. Error"]
@
...
<<make-cout>>=
cout4 <- aster(resp ~ varb + nsloc + ewloc + site * hdct - site,
